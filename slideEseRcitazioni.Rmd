...
X <- rnorm(n=100, mean=0, sd=1) # genero valori casuali dalla dist. normale standard
par(mfrow = c(2,2), mai = c(0.3,0.5,0.2,0.5)) # voglio 4 grafici in una sola finestra
plot(X) # scatter plot (grafico a dispersione)
hist(X) # istogramma
boxplot(X) # scatter plot (grafico a dispersione)
...
par(mfrow = c(2,2), mai = c(0.3,0.5,0.2,0.5)) # voglio 4 grafici in una sola finestra
plot(Y, main="Frequenze assolute") # per oggetti di classe factor = barplot(table(Y))
barplot(round(cumsum(table(Y)/length(Y)),2), main="Frequenze cumulate relative")
plot(Y) # stesso grafico ma aggiungo del testo in blu
...
qqnorm(X,cex=0.5,main="",pch=20) # quant-quanti
...
qqnorm(Y,cex=0.5,main="",pch=20) # quant-quanti
...
# creo y (indipendente da x) e z (proporzionale a x)
Y <- rnorm(1000, mean=20, sd=1)
Z <- X + rnorm(1000, sd = 20)
...
\fontsize{6pt}{12}\selectfont  
```{r message=FALSE,out.width='150px'}
# creo dataframe con 4 variabili
...
Teoria dei campioni, inferenza + altro

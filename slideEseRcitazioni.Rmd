...
- **Giorno 6: Regressione mutlilivello & SEM** \newline Modelli lineari a effetti misti e path analysis
...
- La **regressione lineare** consiste nel determinare il legame tra due variabili attraverso una funzione lineare del tipo \color{red} $Y_i = \beta_0 + \beta_1 X_i + \epsilon_i$ \color{black}

- Tale funzione può essere rappresentata graficamente con una **retta** in cui \newline $\beta_0$ rappresenta l’**intercetta** (valore assunto da Y quando X = 0) e \newline $\beta_1$ il **coefficiente angolare**.
...
\fontsize{6.5pt}{12}\selectfont
Uso il dataset `gambling` dal pacchetto `ADati`. Voglio specificare dei modelli che predicano `frequency` (frequenza di gioco) sulla base di `gender` (genere) e `risk` (percezione del rischio).
```{r }
...
\fontsize{6.5pt}{12}\selectfont
**1. Indipendenza delle osservazioni**: tutte le coppie di errori $\epsilon_i$ e $\epsilon_j$ sono indipendenti per ogni $i \neq j$. Questa non richiede particolari procedure, basta riflettere: le osservazioni nel mio dataset sono tra loro indipendenti?

**2. Indipendenza dei predittori dall'errore**: i **residui** (componente d'errore $\epsilon_i$) del modello non sono associati ai valori dei predittori `gender` e `risk`.
...
\fontsize{6.5pt}{12}\selectfont
**2. Indipendenza dei predittori dall'errore**: i **residui** (componente d'errore $\epsilon_i$) del modello non sono associati ai valori dei predittori `gender` $X_{1i}$ e `risk` $X_{2i}$. In questo caso, non sembrano esserci differenze di genere, né una relazione sostanziale con `risk`.

\fontsize{6.5pt}{12}\selectfont
**3. Varianza costante (omogeneità delle varianze)**: la varianza dei residui (gli errori $\epsilon$) è costante per qualunque valore di $X$. Qui vediamo che la varianza dei residui è leggermente (ma non esageratamente) maggiore nel gruppo dei maschi.
```{r fig.width=8,fig.height=3}
...
\fontsize{6.5pt}{12}\selectfont
**4. Linearità**: il valore atteso dell'errore $\epsilon$ per un dato valore di $X$ è zero. \newline In pratica, i residui devono avere media zero (come in questo caso).

\fontsize{6.5pt}{12}\selectfont
**5. Normalità**: i residui (gli errori $\epsilon$) sono distribuiti normalmente. \newline In questo caso non è un bellissimo fit, ma possiamo accettarlo...
...
\fontsize{7pt}{12}\selectfont
- La **regressione lineare** consiste nel determinare il legame tra due variabili attraverso una funzione lineare del tipo $Y_{ji} =$ \color{blue} $\beta_0$ \color{black} + \color{red} $\beta_1$ \color{black} $X_i + \epsilon_i$ \newline dove \color{blue} $\beta_0$ è l'**intercetta** \color{black} e \color{red} $\beta_1$ è la **pendenza** \color{black}
...
- La **regressione lineare multilivello** permette di analizzare questi casi assumendo che l'intercetta e la pendenza **varino casualmente** da un soggetto all'altro: $Y_{ij} =$ \color{blue} $\beta_0$ \color{black} + \color{red} $\beta_1$ \color{black} $X_{ij} + \epsilon_{ij}$ = \color{blue} $(\beta_0 + \lambda_{0j})$ \color{black} + \color{red} $(\beta_1 + \lambda_{1j})$ \color{black} $X_{ij} + \epsilon_{ij}$
...
Beautiful tutorial: \color{blue} **http://mfviz.com/hierarchical-models**
...
```{r echo=FALSE}
data.frame(subj=c("s1","s2","s3"),age=c(23,24,22),
           RT_trial1=c(386,345,394),RT_trial2=c(389,338,410),RT_trialN=c("...","...","..."),
           meanRT=c(370.5,321.2,397.1))
```

- **foma lunga** (*long form*): una riga per trial (`trialN`), con più righe per soggetto
```{r echo=FALSE}
data.frame(subj=c("s1","s1","s1","s2"),age=c(23,23,23,24),
           trial=c("1","2","...","1"),condition=c("A","A","...","A"),
           RT=c("386","389","...","345"))
```
...
\fontsize{7pt}{12}\selectfont
$Y_{ij} =$ \color{blue} $\beta_0$ \color{black} + \color{red} $\beta_1$ \color{black} $X_{ij} + \epsilon_{ij}$ (*livello 2: between*) \newline = \color{blue} $(\beta_0 + \lambda_{0j})$ \color{black} + \color{red} $(\beta_1 + \lambda_{1j})$ \color{black} $X_{ij} + \epsilon_{ij}$ (*livello 1: within*) \newline Nei modelli multilivello, la varianza della v. dipendente è scomposta in due componenti:
...
data.frame(subj=c("s1","s1","s1","s2"),age=c(23,23,23,24),
           trial=c("1","2","...","1"),condition=c("A","A","...","A"),
           RT=c("386","389","...","345"),
           meanRT=c(370.5,370.5,370.5,321.2),
           RTdev=c(386-370.5,389-370.5,"...",345-321.2))
```
...
boxplot(y ~ time, data=sherifdat) # bivariat.
...
\fontsize{6pt}{12}\selectfont  
6. Si valuti, con un **modello di regressione lineare**, se `time`, `group` e `condition`
siano predittive delle `y`. Quali considerazioni si possono fare osservando
l'output del modello con il comando `summary()`?
...
```{r warning=FALSE,message=FALSE,fig.width=5,fig.height=6}
...
\fontsize{6pt}{12}\selectfont  
8. Si modifichi opportunamente il modello del punto 6 utilizzando degli **effetti random** che
tengano conto del disegno sperimentale e dei dati. Si interpretino i parametri ottenuti.
```{r warning=FALSE,message=FALSE}
...
m <- lmer(y ~ time + condition + (1|group), 
          data = sherifdat)
# intercetta & pendenza random: (time|group)
m2 <- lmer(y ~ time + condition + (time|group), 
           data = sherifdat)
...
```
```{r warning=FALSE,message=FALSE,fig.width=4.5,fig.height=4.2}
par(mfrow=c(2,2))
...
\fontsize{6pt}{12}\selectfont
$Y_{ij} =$ \color{blue} $\beta_0$ \color{black} + \color{red} $\beta_1$ \color{black} $X_{ij} + \epsilon_{ij}$ \newline = \color{blue} $(\beta_0 + \lambda_{0j})$ \color{black} + \color{red} $\beta_1$ \color{black} $X_{ij} + \epsilon_{ij}$ \newline 
...
\fontsize{7pt}{12}\selectfont
La **regressione lineare** consiste nel determinare il legame tra due variabili attraverso una funzione lineare del tipo $Y = \beta_1X_{1} + \beta_2X_{2} + \epsilon$ \newline
...
>- **Variabili endogene** ($Y_2$, $Y_3$): 'causate' all'interno del modello (dipendenti e/o predittori), viene stimato l'errore $\epsilon$
...
S = \left(\begin{array}{cc} 
    S(y_2,y_2),S(y_2,y_3),S(y_2,x_1) \\
    S(y_3,y_2),S(y_3,y_3),S(y_3,x_1) \\
    S(x_1,y_2),S(x_1,y_3),S(x_1,x_1) 
    \end{array}\right)
$$

Obiettivo dei SEM: stimare i parametri $\theta$ che rendano la **matrice di covarianza teorica** $\Sigma$ il più simile possibile alla matrice empirica $S$: \color{red} $H_0: \hat\Sigma(\theta) = \Sigma$ \color{black} \newline \newline dove $\hat\Sigma$ è la matrice riprodotta sulla base dei parametri stimati da $S$, mentre $\Sigma$ è la matrice di covarianza vera.

  \endcol
\endcols

## SEM: Variabili osservate e variabili latenti

\begincols
  \begincol{.5\textwidth}

\fontsize{6pt}{12}\selectfont
SEM con **variabili osservate** ($x$ e $y$): direttamente osservate e misurabili, rappresentate dai quadrati. \newline
```{r echo=FALSE}
tmp <- grViz(
'digraph boxes_and_circles {
  graph [layout = neato]
  ...
  X1->Y2 [label = <&beta;<SUB>21</SUB>>]
  X1->Y3 [label = <&beta;<SUB>31</SUB>>]
  Y2->Y3 [label = <&beta;<SUB>32</SUB>> dir=both]
  e2->Y2
  e3->Y3}')
...
tmp = export_svg(tmp)
tmp = charToRaw(tmp) # flatten
rsvg_png(tmp, "img/semb.png") # saved graph as png in current working directory
```
```{r , echo = FALSE, out.width = "130px",fig.align="center"}
knitr::include_graphics("img/semb.png")
```

SEM con **variabili latenti** ($\xi$, "xi"): ipotetiche, non misurabili direttamente ma ***indicate*** (attraverso i loading $\lambda$, "lambda") dalle $x$, rappresentate dai cerchi. \newline
```{r echo=FALSE}
tmp <- grViz(
'digraph boxes_and_circles {
  graph [layout = neato]
  node [shape = oval]
  E1 [label = <&xi;1> pos="-0.5,0!"]
  E2 [label = <&xi;2> pos="0.5,0!"]
  node [shape = box]
  ...
  # edges
  E1->x1 [label = <&lambda;<SUB>11</SUB>>]
  E1->x2 [label = <&lambda;<SUB>21</SUB>>]
  E1->x3 [label = <&lambda;<SUB>31</SUB>>]
  E2->x4 [label = <&lambda;<SUB>42</SUB>>]
  E2->x5 [label = <&lambda;<SUB>52</SUB>>]
  E2->x6 [label = <&lambda;<SUB>62</SUB>>]
  E1->E2 [label = <&beta;<SUB>21</SUB>>]
  e1->x1
  e2->x2
  e3->x3
  e4->x4
...
\fontsize{6pt}{12}\selectfont
In R, si può usare il pacchetto `lavaan`: 
```{r , eval=FALSE}
library(lavaan)

# modello con variabili osservate
m <- sem(model = 'Y2 ~ X1  # regressione (~)
                  Y3 ~ X1   
                  Y3 ~~ Y2 # covarianza (~~)',
         data = my_data)
...
m <- sem(model = '# modello di misurazione (=~)
                  xi1 =~ x1 + x2 + x3
                  xi2 =~ x4 + x5 + x6
...
Si vuole valutare se le variabili `x1`, `x2` e `x3` sono **predittive dei voti** sui corsi.
...
colnames(finn) <- c("x1","x2","x3","y1","y2")
```
\fontsize{6.5pt}{12}\selectfont  
...
'digraph boxes_and_circles {
  ...
  node [shape = box]
  x1 [label = <x<SUB>1</SUB>> pos="-2,0!"]
  y1 [label = <y<SUB>1</SUB>> pos="-1,-1!"]
  y2 [label = <y<SUB>2</SUB>> pos="0,0!"]
  x2 [label = <x<SUB>2</SUB>> pos="2,0!"]
  y3 [label = <y<SUB>2</SUB>> pos="3,-1!"]
  y4 [label = <y<SUB>3</SUB>> pos="4,0!"]
  node [shape = plaintext]
  e1 [label = <&epsilon;> pos="0,-1!"]
  e2 [label = <&epsilon;>pos="1,0!"]
  e3 [label = <&epsilon;> pos="4,-1!"]
  e4 [label = <&epsilon;>pos="5,0!"]
  # edges
  x1->y1 [label = Recursive]
  x1->y2 
  y1->y2
  x2->y3 [label = Nonrecursive]
  x2->y4
  y3->y4
  y4->y3
  e1->y1
...
tmp = export_svg(tmp)
tmp = charToRaw(tmp) # flatten
rsvg_png(tmp, "img/semr.png") # saved graph as png in current working directory
```
```{r , echo = FALSE, out.width = "170px"}
knitr::include_graphics("img/semr.png")
```

\fontsize{6.5pt}{12}\selectfont
5. Si valuti la **signifcatività statistica** dei parametri. Qual'è l'ipotesi nulla sottostante ciascun
parametro?
...
```{r warning=FALSE, message=FALSE}
library(lavaan)
fit <- sem(model = m, data = finn)
parameterestimates(fit)[,1:7]
```
